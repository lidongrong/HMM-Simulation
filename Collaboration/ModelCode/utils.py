...
def permute_train(permute,est_pi,est_A,est_mu,est_sigma,est_beta):
    '''
    ...
    for i in range(0,len(est_A)):
        # permute est_A
        est_A[i]=est_A[i][permute]
        est_A[i]=est_A[i][:,permute]
        # permute est_pi
    ...
    return est_pi,est_A,est_mu,est_sigma,est_beta
...
def simulation(paths,param_path,param_names,sample_size,covariates,covariate_types,args,iterations,nums,prob):
    '''
    paths: list of path
