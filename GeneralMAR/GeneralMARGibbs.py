...
def sample_A(data,I,A,pi,hidden_state,obs_state,p):
    ...
    for j in range(0,A.shape[0]):
        ...
        state=hidden_state[j]
        temp=p.starmap(emp_trans,[(seq,state,hidden_state,obs_state) for seq in I])
        temp=np.array(temp)
        
        # acquire the empirical tranfer number of the current state
        emp=sum(temp)
...
def sample_pi(I,A,pi,hidden_state,obs_state,p):
    ...
    '''
    temp=p.starmap(emp_first_state,[(seq,hidden_state,obs_state) for seq in I])
    temp=np.array(temp)
    emp=sum(temp)
    ...
    '''
    #Just Testing~
    pi=np.random.dirichlet(1+emp,1)[0]
