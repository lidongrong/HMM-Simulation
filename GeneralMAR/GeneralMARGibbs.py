...
def data_initializer(transition,obs_prob,pi,hidden_state,obs_state):
    ...
    alpha_A=np.array([1 for i in range(0,transition.shape[0])])
    alpha_B=np.array([1 for i in range(0,obs_prob.shape[1])])
    # Dirichlet parameter pi
    alpha_pi=np.array([1 for i in range(0,transition.shape[0])])
    
    A=np.random.dirichlet(alpha_A,transition.shape[0])
    B=np.random.dirichlet(alpha_B,obs_prob.shape[0])
    pi=np.random.dirichlet(alpha_pi,1)[0]
...
# Sample A using the slice sampler
# Have a try, not sure if it works
def sample_A(data,I,A,pi,hidden_state,obs_state,p):
    for j in range(0,A.shape[0]):
        
        # Sample u in slice sampling
        log_p=p.starmap(p_seq,[(seq,A,pi,hidden_state,obs_state) for seq in I])
        exp_log_p=np.exp(log_p)
        u=np.random.uniform(np.zeros(len(exp_log_p)),exp_log_p)
        
        # Sample rows of A by rejection sampling
        new_A=A.copy()
        new_A[j,:]=np.random.dirichlet([1 for i in range(0,len(new_A[j,:]))])
        while np.all(np.exp(np.array(p.starmap(p_seq,[(seq,A,pi,hidden_state,obs_state) for seq in I])))<u):
            new_A[j,:]=np.random.dirichlet([1 for i in range(0,len(new_A[j,:]))])
        A[j,:]=new_A[j,:]
...
# A torch version for computing the prob of first observable state
# input pi, A shall be tensor-type data
def torch_p_first_state(seq,A,pi,hidden_state,obs_state):
    indexer=np.where(seq!='None')[0]
    # In case the whole seq is missing
    if indexer.size==0:
    ...
    pos=np.where(hidden_state==seq[indexer[0]])[0][0]
    log_p=torch.log(torch.dot(pi,torch.matrix_power(A,indexer[0])[:,pos]))
    
    return log_p

# Sample pi by proposing a langevin jump
# step size epsilon=0.05
def sample_pi(I,A,pi,hidden_state,obs_state,p):
    # convert pi and A into tensor
    ...
    log_p=torch.tensor(p.starmap(torch_p_first_state,[(seq,A,pi,hidden_state,obs_state) for seq in I]),requires_grad=True)
    ones=torch.ones(len(log_p))
    # pdf on the full data
    log_p=torch.dot(ones,log_p)
    
    # stepsize
    epsilon=0.05
    
    log_p.backward()
    new_pi=pi+epsilon*epsilon*0.5*pi.grad+epsilon*torch.from_numpy(np.random.normal(0,1,len(pi)))
    log_new_p=torch.dot(torch.ones(len(log_p),torch.tensor(p.starmap(torch_p_first_state,[(seq,A,new_pi,hidden_state,obs_state) for seq in I]),requires_grad=True)))
    
    if sum(new_pi)>1 or torch.any(new_pi<=0):
        return pi.numpy()
    else:
        r=log_new_p-log_p+stats.multivariate_normal.pdf(pi.numpy(),new_pi.numpy(),np.eye(len(pi)))-stats.multivariate_normal.pdf(new_pi.numpy(),pi.numpy(),np.eye(len(pi)))
        r=min(0,r)
        u=np.random.uniform(0,1,1)[0]
        if np.log(u)<r:
            return new_pi.numpy()
