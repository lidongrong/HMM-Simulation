...
def sample_A(data,I,A,pi,hidden_state,obs_state,p):
    ...
    for i in range(0,A.shape[0]):
        ...
        new_A=A.copy()
        ratio=A[i,index]/sum(A[i,index])
        
        new_ratio=np.random.beta(2,1/ratio[0],1)[0]
        new_ratio=[new_ratio,1-new_ratio]
        
        new_A[i,index[0]]=new_ratio[0]*sum(A[i,index])
        new_A[i,index[1]]=new_ratio[1]*sum(A[i,index])
        ...
        if log_p==np.inf:
            ...
            r=log_new_p+np.log(stats.beta.pdf(ratio[0],2,1/new_ratio[0]))-log_p-np.log(stats.beta.pdf(new_ratio[0],2,1/ratio[0]))
            r=min(0,r)
...
def sample_pi(I,A,pi,hidden_state,obs_state,p):
    ...
    ratio=pi[index]/sum(pi[index])
        
    new_ratio=np.random.beta(2,1/ratio[0],1)[0]
    new_ratio=[new_ratio,1-new_ratio]
        
    new_pi[index[0]]=new_ratio[0]*sum(pi[index])
    new_pi[index[1]]=new_ratio[1]*sum(pi[index])
