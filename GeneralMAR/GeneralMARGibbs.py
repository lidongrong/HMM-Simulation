...
def data_initializer(transition,obs_prob,pi,hidden_state,obs_state):
    ...
    # Construct Data
    hidden_data=Sampling.Hidden_Generator(MC,20,3000)
...
def sample_B(data,I,B,hidden_state,obs_state):
    for j in range(0,B.shape[0]):
        ...
        n1=np.sum(np.logical_and(I==hidden_state[j],data==obs_state[0]))
        n2=np.sum(np.logical_and(I==hidden_state[j],data==obs_state[1]))
        n3=np.sum(np.logical_and(I==hidden_state[j],data==obs_state[2]))
        n4=np.sum(np.logical_and(I==hidden_state[j],data==obs_state[3]))
        n5=np.sum(np.logical_and(I==hidden_state[j],data==obs_state[4]))
