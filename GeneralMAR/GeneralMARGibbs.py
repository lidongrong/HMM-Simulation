...
def sample_A(data,I,A,pi,hidden_state,obs_state,p):
    ...
    for i in range(0,A.shape[0]):
        for j in range(0,A.shape[1]):
            # acquire the parameter from the last iteration
            new_A=A.copy()
            new_prop=np.random.beta(2,1/A[i,j])
            new_A[i,:]=A[i,:]*(1-new_prop)/(1-A[i,j])
            new_A[i,j]=new_prop
            ...
            if log_p==np.inf:
                ...
                r=log_new_p+np.log(stats.beta.pdf(A[i,j],2,1/new_A[i,j]))-log_p-np.log(stats.beta.pdf(new_A[i,j],2,1/A[i,j]))
                #r=log_new_p+np.log(stats.dirichlet.pdf(A[j,:],emp+1))-log_p-np.log(stats.dirichlet.pdf(new_A[j,:],emp+1))
...
def sample_pi(I,A,pi,hidden_state,obs_state,p):
    for i in range(0,len(pi)):
        new_pi=pi.copy()
        new_prop=np.random.beta(2,1/pi[i])
        new_pi=pi*(1-new_prop)/(1-pi[i])
        new_pi[i]=new_prop
