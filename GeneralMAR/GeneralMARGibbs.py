...
def sample_A(data,I,A,pi,hidden_state,obs_state,p):
    ...
    for i in range(0,A.shape[0]):
        ...
        index=np.random.choice(A.shape[1],2,replace=False)
        new_A=A.copy()
        propose=np.random.dirichlet((A[i,index[0]]/sum(A[i,index])+1,A[i,index[1]]/sum(A[i,index])+1),1)[0]
        new_propose=(A[i,index[0]]+A[i,index[1]])*propose
        new_A[i,index[0]]=new_propose[0]
        new_A[i,index[1]]=new_propose[1]
        ...
        if log_p==np.inf:
            ...
            r=log_new_p-log_p+np.log(stats.dirichlet.pdf(A[i,index]/sum(A[i,index]),propose+1))-log_p-np.log(stats.dirichlet.pdf(new_A[i,index]/sum(new_A[i,index]),A[i,index]/sum(A[i,index])+1))
            r=min(0,r)
...
def sample_pi(I,A,pi,hidden_state,obs_state,p):
    ...
    new_pi=pi.copy()
    propose=np.random.dirichlet((pi[index[0]]/sum(pi[index])+1,pi[index[1]]/sum(pi[index])+1),1)[0]
    new_propose=(pi[index[0]]+pi[index[1]])*propose
    new_pi[index[0]]=new_propose[0]
    new_pi[index[1]]=new_propose[1]
    ...
    # Metropolis Step
    r=log_new_p-log_p+np.log(stats.dirichlet.pdf(pi[index]/sum(pi[index]),propose+1))-log_p-np.log(stats.dirichlet.pdf(pi[index]/sum(pi[index]),pi[index]/sum(pi[index])+1))
    r=min(0,r)
