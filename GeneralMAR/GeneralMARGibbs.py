...
# return a vector of the first state of a specific sequence
# seq: a specific hidden sequence
def emp_first_state(seq,hidden_state,obs_state):
    n=np.zeros(len(hidden_state))
    indexer=np.where(seq!='None')[0]
...
# Sample pi using slice sampling+ Accept and reject sampling
def sample_pi(I,A,pi,hidden_state,obs_state,p):
    
    # Sample u in slice sampler
    log_p=p.starmap(p_first_state,[(seq,A,pi,hidden_state,obs_state) for seq in I])
    log_p=np.array(log_p)
    exp_log_p=np.exp(log_p)
    u=np.random.uniform(np.zeros(len(exp_log_p)),exp_log_p)
    
    #Sample pi in slice sampler by rejection sampling
    new_pi=np.random.dirichlet([1 for i in range(0,len(pi))])
    
    while np.all(np.exp(np.array(p.starmap(p_first_state,[(seq,A,new_pi,hidden_state,obs_state) for seq in I])))<u):
        new_pi=np.random.dirichlet([1 for i in range(0,len(pi))])
