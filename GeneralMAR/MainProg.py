...
if __name__=='__main__':
    ...
    for i in range(0,10):
        ...
        post_A,post_B,post_pi,latent_seq,log_prob=parallel_Gibbs(data,I,A,B,pi,10,hidden_state,obs_state,p)
        out_obj.append(Out(data,post_A,post_B,post_pi,latent_seq,log_prob,hidden_data))
        
        #make sure that there is no renamed file
        if os.path.exists(f'Experiment{i}'):
            os.remove(f'Experiment{i}')
