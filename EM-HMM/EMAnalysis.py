...
def result_analysis(color_bar,output,num,transition,obs_prob,pi,hidden_state,obs_state,p):
    ...
    for j in range(0,num):
        ...
        plt.subplot(row,2,j+1)
        log_obs=np.array(log_obs)
        plt.plot(np.arange(50,len(log_obs)),log_obs[50:],color_bar[j],label=f'Experiment{k}')
        plt.plot(np.arange(50,len(log_obs)),np.repeat(true_log_prob,len(log_obs)-50),'black',label='True Log Prob')
        plt.xlabel('iteration')
        plt.legend(loc='best')
