# -*- coding: utf-8 -*-
"""
Created on Thu Jan 27 15:26:58 2022
...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 26 13:43:33 2022
@author: s1155151972
"""
...
# -*- coding: utf-8 -*-
"""
Try estimate the full latent sequence
"""
...
# check if the vector is of the desired type
# to prevent label switching problem
def checker():
    pass
...
def p_seq(seq,obs,A,pi,hidden_state,obs_state):
    indexer=np.where(obs!='None')[0]
    log_p=0
...
# switch the label of a hidden sequence 
# label is a vector that can switche the hidden state
def switch_seq(hidden_seq,hidden_state,label):
    z_pos=np.array([hidden_loc(hidden_seq[k],hidden_state) for k in range(0,len(hidden_seq))])
    
    # permute the labels
    new_z_pos=np.array([label[k] for k in z_pos])
    switched_z=hidden_state[new_z_pos]
...
def p_evaluator(A,B,pi,I,data,hidden_state,obs_state,p):    
    ...
    log_z=sum(p.starmap(p_seq,[(I[i],data[i],A,pi,hidden_state,obs_state) for i in range(0,I.shape[0])]))
...
def parallel_Gibbs(data,I,A,B,pi,n,hidden_state,obs_state,p):
    ...
    for i in range(0,n):
        ...
        sa=np.array(np.flip(sorted(pi)))
        #sa=np.array(sorted(B[:,B.shape[1]-1]))
        
        # random permutation sampler
        # prevent label switching problem
        if not np.all(sa ==pi):
            switch=np.array([np.argwhere(pi==i)[0,0] for i in sa])
            print(switch)
            A=A[switch,:]
            A=A[:,switch]
            B=B[switch,:]
            ...
            I=p.starmap(switch_seq,[(I[i],hidden_state,switch) for i in range(0,I.shape[0])])
            I=np.array(I)
